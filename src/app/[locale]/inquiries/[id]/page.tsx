...
export default async function InquiryDetailPage({ params }: Props) {
  ...
  const { data: replies } = await supabase
    .from('inquiry_replies').select('*, profiles(full_name, role)').eq('inquiry_id', id).order('created_at', { ascending: true })
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-12">
      ...
      <div className="mb-10">
        <h1 className="text-3xl font-light mb-3" style={{ fontFamily: 'var(--font-cormorant)' }}>{inquiry.title}</h1>
        <p className="text-[10px] text-[#aaa] tracking-widest mb-6">
          {new Date(inquiry.created_at).toLocaleDateString(locale === 'ko' ? 'ko-KR' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
        </p>
        <p className="text-sm text-[#666] font-light leading-relaxed whitespace-pre-wrap">{inquiry.content}</p>
      </div>
              ...
              <div key={reply.id} className={`pl-4 ${isAdmin ? 'border-l-2 border-[#1a1a1a]' : 'border-l border-[#e8e4de]'}`}>
                <div className="flex justify-between items-center mb-2">
                  <p className="text-[10px] tracking-[0.15em] uppercase text-[#888]">
                    {isAdmin ? t('support') : (reply.profiles as any)?.full_name ?? t('you')}
                  </p>
                  <p className="text-[10px] text-[#aaa]">
                    {new Date(reply.created_at).toLocaleDateString(locale === 'ko' ? 'ko-KR' : 'en-US', { month: 'short', day: 'numeric' })}
                  </p>
                </div>
                <p className="text-sm text-[#444] font-light leading-relaxed whitespace-pre-wrap">{reply.content}</p>
              </div>
            )
          })}
        </div>
