    ...
    <main className="max-w-4xl mx-auto px-6 py-12">
      ...
      <div className="space-y-3">
        {orders?.map((order) => {
          ...
          return (
            <Link key={order.id} href={`/${locale}/orders/${order.id}`} className="flex items-center justify-between bg-white border border-[#e8e4de] px-6 py-4 hover:border-[#1a1a1a] transition-colors group">
        ...
        {(!orders || orders.length === 0) && (
          <p className="text-center text-[#aaa] py-20 text-sm">{t('noOrders')}</p>
        )}
      </div>
    </main>
  )
