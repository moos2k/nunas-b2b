...
export default async function AdminInquiryDetailPage({ params }: Props) {
  ...
  const [{ data: customer }, { data: replies }] = await Promise.all([
    supabase.from('profiles').select('full_name, company').eq('id', inquiry.customer_id).single(),
    supabase.from('inquiry_replies').select('*, profiles(full_name, role)').eq('inquiry_id', id).order('created_at', { ascending: true }),
  ])
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-10">
              ...
              <div key={reply.id} className={`rounded-lg p-5 ${isAdmin ? 'bg-blue-50 border border-blue-100' : 'bg-gray-50 border'}`}>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold">
                    {isAdmin ? '🏢 Support' : (reply.profiles as any)?.full_name ?? 'Customer'}
                  </span>
      ...
      <AdminReplyForm inquiryId={id} authorId={profile.id} currentStatus={inquiry.status} />
    </main>
  )
}
